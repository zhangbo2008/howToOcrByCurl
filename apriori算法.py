...
# 构造数据
def loadDataSet():
    return [[1, 3, 4], [2, 3, 5], [1, 2, 3, 5], [2, 5],[1, 3, 2],[1, 3, 2],[1, 3, 2]]
...
def aprioriGen(Lk, k):  #感觉用collection里面的组合来进行操作代码更简单啊.
    retList = []
    lenLk = len(Lk)
...
'''
aprior这个函数是最终的接口函数,他返回了dataSet集合里面所有大于minSupport的组合和对应的频率.
'''
...
#上面得到了所有组合对应的得分,只保留超过minSupport的得分.
...
def generateRules(L, supportData, minConf=0.7):  # supportData 是一个字典
    ...
    for i in range(1, len(L)):  # 从为2个元素的集合开始
        for freqSet in L[i]:
            ...
            if (i > 1):  #如果集合里面的数量大于2个,比如是3个那么就需要进行 复杂的处理,也就是下面的rulesFromConseq函数.
...
def calcConf(freqSet, H, supportData, brl, minConf=0.7):
    prunedH = []  # 创建一个新的列表去返回  #这里面就是 如果集合是(2,3) 那么我们就计算 2->3和3->2 的条件概率分别是多少即可.
    for conseq in H:
...
def rulesFromConseq(freqSet, H, supportData, brl, minConf=0.7):
    m = len(H[0])
    if (len(freqSet) > (m + 1)):  # 尝试进一步合并  #如果
        Hmp1 = aprioriGen(H, m + 1)  # 将单个集合元素两两合并,那么需要重新生成子集和.
        Hmp1 = calcConf(freqSet, Hmp1, supportData, brl, minConf)
        if (len(Hmp1) > 1):  # need at least two sets to merge, 这里面就是利用了aprior的性质,如果子关联的条件概率足够大,那么嗨需要继续监测更大的推广,因为更大的推广的条件概率会更小,可能会满足,但是如果当前发现Hmp1==[],那么根据性质就不用继续递归了.整个算法的核心就在这个if 语句中!
            rulesFromConseq(freqSet, Hmp1, supportData, brl, minConf)


#下面计算关联系数0.7以上的.结果.
rules = generateRules(L, suppData, minConf=0.7)
print("下面打印最终的规则: 条件set,结果set,条件概率:也就是 条件+结果的概率除以条件的概率")
print(rules)
